...
//---------------------------------------------------------------------------------
    ...
    ssize_t writeNum = write(fd, "sram ext test1: OK\n", BUFFER_SIZE);
    if (writeNum == -1)
    {
        perror("write: Error");
        goto wait;
    ...
    fseek(fp, SKIP_CNT, SEEK_SET);
    fputs("sram ext test2: OK\n", fp);
    fseek(fp, SKIP_CNT, SEEK_SET);
